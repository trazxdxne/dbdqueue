...
pub fn update_hosts(selected_aws_regions: Option<&[String]>, verbose: bool) -> UpdateHostsResult {
    ...
    let new_content = match build_hosts_content(hosts_path, selected_aws_regions) {
        ...
        Err(e) => {
            ...
            if verbose {
                let err_hdr = if is_ru { "Ошибка создания содержимого hosts:" } else { "Error generating hosts file content:" };
    ...
    if current_content == new_content {
        if verbose {
            let msg = if is_ru { "Файл hosts уже актуален." } else { "Hosts file is already up to date." };
    ...
    if fs::write(hosts_path, &new_content).is_ok() {
        if verbose {
            let msg = if is_ru { "Блокировка регионов успешно обновлена!" } else { "Successfully updated region locks!" };
            println!("\x1b[92m{}\x1b[0m", msg);
        }
        flush_dns_cache();
    ...
    if let Err(e) = fs::write(&temp_path, &new_content) {
        let err_msg = format!("{}", e);
        if verbose {
            let err_hdr = if is_ru { "Не удалось создать временный файл hosts:" } else { "Failed to create temporary hosts file:" };
            eprintln!("\x1b[91m{}\x1b[0m {}", err_hdr, err_msg);
        }
        return UpdateHostsResult::Error(err_msg);
    }
...
pub fn update_hosts(selected_aws_regions: Option<&[String]>, verbose: bool) -> UpdateHostsResult {
        ...
        .spawn()
    {
        ...
        Err(e) => {
            let err_msg = format!("{}", e);
            if verbose {
                let err_hdr = if is_ru { "Ошибка запуска pkexec:" } else { "Error executing pkexec:" };
                eprintln!("\x1b[91m{}\x1b[0m {}", err_hdr, err_msg);
            }
            return UpdateHostsResult::Error(err_msg);
        }
    ...
    if let Some(mut stdin) = child.stdin.take()
        && let Err(e) = stdin.write_all(new_content.as_bytes()) {
            let err_msg = format!("{}", e);
            if verbose {
                let err_hdr = if is_ru { "Ошибка записи в stdin pkexec:" } else { "Error writing to pkexec stdin:" };
                eprintln!("\x1b[91m{}\x1b[0m {}", err_hdr, err_msg);
            }
            return UpdateHostsResult::Error(err_msg);
        }
    ...
    match child.wait_with_output() {
        Ok(output) => {
            if output.status.success() {
                ...
                if verbose {
                    let fail_hdr = if is_ru { "Не удалось записать в /etc/hosts" } else { "Failed to write to /etc/hosts" };
                    eprintln!(
                        "\x1b[91m{} (code {:?}):\x1b[0m\n{}",
                        fail_hdr,
                        output.status.code(),
        ...
        Err(e) => {
            let err_msg = format!("{}", e);
            if verbose {
                let err_hdr = if is_ru { "Ошибка ожидания pkexec:" } else { "Error waiting for pkexec:" };
                eprintln!("\x1b[91m{}\x1b[0m {}", err_hdr, err_msg);
            }
            UpdateHostsResult::Error(err_msg)
        }
